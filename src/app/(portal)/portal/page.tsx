  ...
  return (
    <div className="animate-fade-in space-y-7">
      <header className="pt-1">
        ...
        <p className="mt-2 max-w-xl text-base leading-relaxed text-muted">Acá podés ver si está todo bien y qué necesitás hacer.</p>
      </header>
      ...
      {!project ? (
        ...
        <section className="relative overflow-hidden rounded-3xl border border-success/25 bg-surface p-6 sm:p-8">
          <div className="relative max-w-xl">
            ...
            <h2 className="mt-4 text-2xl font-semibold tracking-tight sm:text-3xl">Tu web está funcionando correctamente</h2>
            {data.domain?.domain && <p className="mt-2 text-sm text-muted">{data.domain.domain}</p>}
            <p className="mt-2 text-base text-muted">No necesitás hacer nada por ahora.</p>
            <a href={data.hosting!.production_url!} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex">
              <Button size="lg"><ExternalLink size={17} /> Ver mi web</Button>
            </a>
          </div>
        </section>
      ) : (
      ...
      <section className="rounded-2xl border border-accent/20 bg-accent-soft/50 p-5 sm:flex sm:items-center sm:justify-between sm:gap-5">
        <div className="flex items-start gap-3">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-accent text-white"><LifeBuoy size={20} /></span>
          <div>
            <h2 className="text-lg font-semibold">¿Necesitás ayuda?</h2>
            <p className="mt-1 text-base leading-relaxed text-muted">Escribile a Mateo. No necesitás explicar nada de forma técnica.</p>
          </div>
        </div>
        <Link href="/portal/solicitudes/nueva" className="mt-4 block shrink-0 sm:mt-0">
          <Button size="lg" className="w-full">Hablar con Mateo <ArrowRight size={16} /></Button>
        </Link>
      </section>

      <section>
        <div className="mb-3 flex items-end justify-between gap-3">
          <div><h2 className="text-lg font-semibold tracking-tight">Todo lo demás</h2><p className="mt-1 text-sm text-muted">Accesos directos a tu información.</p></div>
        </div>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <QuickLink href="/portal/solicitudes" icon={LifeBuoy} label="Mis consultas" detail="Ver respuestas de Mateo" />
